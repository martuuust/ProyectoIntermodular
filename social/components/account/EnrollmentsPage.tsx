  ...
  useEffect(() => {
    try {
      ...
      if (allEnrollmentsData) {
        ...
        const enrichedData = userEnrollments.map(enrollment => {
            const camp = CAMPS_DATA.find(c => c.id === enrollment.campId);
            return camp ? { ...enrollment, camp } : null;
        }).filter((e): e is EnrichedEnrollment => e !== null);
